...
class _RoomScrollView extends State<RoomScrollView> {
                                    ...
                                    child: Stack(
                                      children: [
                                        ...
                                        Positioned.fill(
                                            child: FutureBuilder(
                                          future:
                                              _fetchRoomType(roomItems[index]),
                                          builder: (context, snapshot) {
                                            if (snapshot.connectionState ==
                                              ...
                                              return Row(
                                                children: [
                                                  Padding(padding: const EdgeInsets.only(left: 20), child: TextButton(
                                                    child: Text(
                                                      roomItems[index],
                                                  ...
                                                  Padding(padding: const EdgeInsets.only(right: 20), child: Text(
                                                    'Max Points: ${calculateMaxPoints(roomType!)}',
                                                    style: const TextStyle(
                                                        color: Colors.white),
                                                  ))
  ...
  Future<RoomType?> _fetchRoomType(String roomName) async {
    RoomType roomType;
    ...
    if (user != null) {
      ...
      if (querySnapshot.docs.isNotEmpty) {
        ...
        if (roomSnapshot.exists) {
          ...
          roomType =
              RoomType.values.firstWhere((e) => e.toString() == roomTypeString);
          return roomType;
        }
      }
    }
  ...
  Future<void> _fetchRoomItems() async {
    ...
    if (user != null) {
      String userId = user.uid;
      QuerySnapshot roomsQuery = await FirebaseFirestore.instance
          .collection('users')
          .doc(userId)
          .collection('housings')
          .doc(selectedHousingId)
      ...
      child: DropdownButton<String>(
        ...
        onChanged: (String? newValue) async {
          if (newValue != null) {
            ...
            if (user != null) {
              String userId = user.uid;
              DocumentSnapshot housingDoc = await FirebaseFirestore.instance
                  .collection('users')
                  .doc(userId)
                  .collection('housings')
                  .doc(newValue)
  ...
  Future<void> _fetchHousingDropdownItems() async {
    ...
    if (user != null) {
      String userId = user.uid;
      QuerySnapshot housingQuery = await FirebaseFirestore.instance
          .collection('users')
          .doc(userId)
          .collection('housings')
          .get();
      ...
      List<DropdownMenuItem<String>> items = housingQuery.docs.map((doc) {
        return DropdownMenuItem<String>(
          value: doc.id,
          child: Text(doc['housingName']),
        );
      }).toList();

      if (items.isNotEmpty) {
        String defaultHousingId = items.first.value!;
        DocumentSnapshot defaultHousingDoc = await FirebaseFirestore.instance
            .collection('users')
            .doc(userId)
            .collection('housings')
            .doc(defaultHousingId)
            .get();
